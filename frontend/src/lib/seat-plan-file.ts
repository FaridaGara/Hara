...
export const FILE_ERROR =
  "Fayl açıla bilmir. Şifrəsiz PDF, PNG və ya JPG formatında, maksimum 10 MB fayl seç.";
export async function openPlanFile(
  file: File,
): Promise<{ pdf: PDFDocumentProxy | null; image: string; pages: number }> {
  if (
  ...
  if (file.type === "application/pdf") {
    ...
    if (pdf.numPages > 100) {
      ...
      throw new Error("Maksimum 100 səhifəlik PDF seç.");
    }
  ...
  try {
    ...
    if (!ctx) throw new Error(FILE_ERROR);
    ctx.fillStyle = "white";
...
export async function renderPlanPage(
  ...
  const viewport = page.getViewport({
    scale: 1400 / Math.max(original.width, original.height),
  });
  const canvas = document.createElement("canvas");
